...
g.__STAGING__ = g.__STAGING__ || __STAGING__
g.__PROD__ = g.__PROD__ || __PROD__
g.__REPORT_COVERAGE__ = g.___REPORT_COVERAGE__ || (__TEST__ && !!config.test_watch)
g.process = g.process || {}
...
if (config.compiler_lint) {
  ...
  // TODO: we should throw if an .eslintrc file is not found.
...
webpackConfig.module.loaders.push({
  ...
  loader: 'babel',
  query: require('./internal/_babel-config').__do_not_change_or_you_will_be_fired__,
})
...
if (config.compiler_provide && typeof config.compiler_provide === 'object') {
  webpackConfig.plugins.push(
    new webpack.ProvidePlugin(config.compiler_provide)
  )
}
